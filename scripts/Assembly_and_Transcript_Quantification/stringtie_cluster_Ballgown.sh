...
if [ ! $LIBRARY_TYPE ]
then
    stringtie ${BAM} -v -p 10 -G ${GTF} --rf -e -B -o ${OUTPUT}"/stringtieB_outfile.gtf"
elif [ $LIBRARY_TYPE == 'fr-secondstrand' ]
then
    stringtie ${BAM} -v -p 10 -G ${GTF} --fr -e -B -o ${OUTPUT}"/stringtieB_outfile.gtf"
elif [ $LIBRARY_TYPE == 'fr-firststrand' ]
then
    stringtie ${BAM} -v -p 10 -G ${GTF} --rf -e -B -o ${OUTPUT}"/stringtieB_outfile.gtf"
elif [ $LIBRARY_TYPE == 'fr-unstranded' ]
then
    stringtie ${BAM} -v -p 10 -G ${GTF} -e -B -o ${OUTPUT}"/stringtieB_outfile.gtf"
fi
